...
from peft.import_utils import require_spops
...
class SpMMFunction(torch.autograd.Function):
    ...
    def backward(ctx, dLdC):
        ...
        require_spops()
        from spops import csr_transpose, sddmm, spmm
...
class SpMMTFunction(torch.autograd.Function):
    ...
    @staticmethod
    @torch.cuda.amp.custom_fwd
    def forward(ctx, AT_val, AT_row_offsets, AT_row_indices, AT_col_indices, B, M, A_val, A_row_offsets, A_row_indices, A_col_indices):  # noqa: E501
        require_spops()
        from spops import csr_transpose, spmm
